...
fn test_fast_exp_accuracy() {
    ...
    for &x in &inputs {
        ...
        let relative_error = if std_exp != 0.0 { absolute_error / std_exp } else { 0.0 };
        ...
        // Write to file
        writeln!(file, "{},{},{},{},{}", 
                 x, std_exp, fast_exp, relative_error, absolute_error).unwrap();
    }

    // Print summary
    println!("Fast exp accuracy test completed. Check fast_exp_accuracy.txt");
    println!("Maximum relative error: {:.6} at x = {}", max_relative_error, max_error_input);
    ...
    assert!(max_relative_error < 0.35, "Maximum relative error exceeds 35%");
}
