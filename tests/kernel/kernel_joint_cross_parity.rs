...
fn kernel_joint_python_parity_2d(#[case] kernel: &str, #[case] bw: f64, #[case] size: usize, #[case] seed: u64) {
...
fn kernel_cross_python_parity_1d(#[case] kernel: &str, #[case] bw: f64, #[case] size: usize, #[case] seed: u64) {
...
fn kernel_cross_python_parity_2d(#[case] kernel: &str, #[case] bw: f64, #[case] size: usize, #[case] seed: u64) {
    ...
    for i in 0..size { p_vec.push([p_x[i], p_y[i]]); }
    let mut q_vec = Vec::new();
    for i in 0..size { q_vec.push([q_x[i], q_y[i]]); }

    let p_data = Array2::from_shape_vec((p_vec.len(), 2), p_vec.into_iter().flatten().collect()).unwrap();
    let q_data = Array2::from_shape_vec((q_vec.len(), 2), q_vec.into_iter().flatten().collect()).unwrap();
