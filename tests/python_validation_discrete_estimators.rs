...
fn validate_discrete_against_python() {
    ...
    let locals_py =
        python::calculate_local_entropy(&data.to_vec(), "discrete", &[]).expect("python failed");
...
fn validate_miller_madow_against_python() {
    ...
    // Python
    let h_py =
        python::calculate_entropy(&data.to_vec(), "miller_madow", &[]).expect("python failed");
    let locals_py = python::calculate_local_entropy(&data.to_vec(), "miller_madow", &[])
...
fn validate_shrink_against_python() {
    ...
    let locals_py =
        python::calculate_local_entropy(&data.to_vec(), "shrink", &[]).expect("python failed");
...
fn validate_grassberger_against_python() {
    ...
    let locals_py =
        python::calculate_local_entropy(&data.to_vec(), "grassberger", &[]).expect("python failed");
    // The Python global result() reduces local values to a scalar; request global too
    let h_py =
        python::calculate_entropy(&data.to_vec(), "grassberger", &[]).expect("python failed");
...
fn validate_zhang_against_python() {
    ...
    let locals_py =
        python::calculate_local_entropy(&data.to_vec(), "zhang", &[]).expect("python failed");
...
fn validate_chao_wang_jost_against_python() {
    ...
    let h_rust = rust.global_value();
    let h_py = python::calculate_entropy(&data.to_vec(), "chao_wang_jost", &[])
        .or_else(|_| {
...
fn validate_bayes_against_python_laplace() {
    ...
    let h_py = python::calculate_entropy(
        &data.to_vec(),
        "bayes",
        &[("alpha".to_string(), "\"laplace\"".to_string())],
    )
    .expect("python failed");
...
fn validate_discrete_python_datasets() {
    ...
    let data_cases: Vec<Vec<i32>> =
        vec![vec![1, 0, 1, 0], vec![1, 2, 3, 4, 5], vec![1, 1, 1, 1, 1]];
    for data in data_cases.into_iter() {
        ...
        let locals_py =
            python::calculate_local_entropy(&data, "discrete", &[]).expect("python failed");
        assert_entropy_values_close(
            h_rust,
            h_py,
            1e-10,
            1e-6,
            "discrete (python datasets) global",
        );
        for (lr, lp) in locals_rust.iter().zip(locals_py.iter()) {
...
fn validate_miller_madow_python_datasets() {
    ...
    for data in data_cases.into_iter() {
        ...
        let locals_py =
            python::calculate_local_entropy(&data, "miller_madow", &[]).expect("python failed");
        assert_entropy_values_close(
            h_rust,
            h_py,
            1e-10,
            1e-6,
            "miller_madow (python datasets) global",
        );
        for (lr, lp) in locals_rust.iter().zip(locals_py.iter()) {
            assert_entropy_values_close(
                *lr,
                *lp,
                1e-10,
                1e-6,
                "miller_madow (python datasets) local",
            );
        }
    }
}
...
fn validate_bayes_with_k_parameter_against_python() {
    ...
    let rust_k3 = BayesEntropy::new(
        ndarray::Array1::from(data.clone()),
        AlphaParam::Laplace,
        Some(3),
    );
    let h_rust_k3 = rust_k3.global_value();
    let h_py_k3 = python::calculate_entropy(
        &data,
        "bayes",
        &[("alpha".into(), "1.0".into()), ("K".into(), "3".into())],
    )
    .expect("python failed");
    assert_entropy_values_close(h_rust_k3, h_py_k3, 1e-10, 1e-6, "bayes K=3 global");

    let rust_k5 = BayesEntropy::new(
        ndarray::Array1::from(data.clone()),
        AlphaParam::Laplace,
        Some(5),
    );
    let h_rust_k5 = rust_k5.global_value();
    let h_py_k5 = python::calculate_entropy(
        &data,
        "bayes",
        &[("alpha".into(), "1.0".into()), ("K".into(), "5".into())],
    )
    .expect("python failed");
...
fn validate_shrink_python_datasets() {
    ...
    let datasets: Vec<Vec<i32>> = vec![vec![1, 1, 1, 1, 1], vec![1, 0, 1, 0], vec![1, 2, 3, 4, 5]];
    for data in datasets.into_iter() {
        ...
        let locals_py =
            python::calculate_local_entropy(&data, "shrink", &[]).expect("python failed");
...
fn validate_chao_shen_python_datasets() {
    ...
    let datasets: Vec<Vec<i32>> = vec![vec![1, 2, 3, 4, 5], vec![1, 1, 2, 3, 3, 4, 5]];
    for data in datasets.into_iter() {
        ...
        assert_entropy_values_close(
            ...
            1e-10,
            1e-6,
            "chao_shen (python datasets) global",
        );
    }
}
...
fn validate_grassberger_small_dataset_against_python() {
    ...
    let h_rust = rust.global_value();
    let locals_py =
        python::calculate_local_entropy(&data, "grassberger", &[]).expect("python failed");
