...
fn ordinal_basic_patterns_and_entropy() {
    ...
    let ord = OrdinalEntropy::new(series.clone(), 2);
...
fn ordinal_tie_handling_stable() {
    ...
    let ord = OrdinalEntropy::new(series, 2);
    let h = ord.global_value();
...
fn ordinal_insufficient_length_yields_zero_entropy() {
    ...
    let ord = OrdinalEntropy::new(series, 4);
    assert_eq!(ord.local_values().len(), 0);
    assert_abs_diff_eq!(ord.global_value(), 0.0, epsilon = 1e-12);
}

#[test]
fn ordinal_order_limit_no_panic_over_12() {
    // With remapped codes, orders up to 20 should work (Lehmer code u64 safety limit)
    let series: Array1<f64> = Array1::from((0..30).map(|x| x as f64).collect::<Vec<_>>());
    // order=13 should no longer panic
    let ord = OrdinalEntropy::new(series.clone(), 13);
    // Ensure we produced the expected number of windows: n - (m-1)
    let expected_windows = series.len() - (13 - 1);
    assert_eq!(ord.local_values().len(), expected_windows);
...
fn ordinal_equivalence_with_discrete_on_codes() {
    ...
    let ord = OrdinalEntropy::new(series.clone(), 3);
    let local = ord.local_values();
    let h_ord = ord.global_value();

    // Recompute codes the same way and feed into DiscreteEntropy
    // Use the canonical symbolization approach
    let codes = symbolize_series_compact(&series, 3, 1, true);
