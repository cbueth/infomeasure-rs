...
fn chao_wang_jost_entropy_basic() {
    ...
    for &x in [1,1,1,2,2,3,5,8].iter() { *counts.entry(x).or_insert(0) += 1; }
    let n = 8usize;
    let mut f1 = 0usize; let mut f2 = 0usize;
    for &c in counts.values() { if c == 1 { f1 += 1; } if c == 2 { f2 += 1; } }
    ...
    for &c in counts.values() {
        if c >= 1 && c < n { cwj += (c as f64) * (dg_n - digamma(c as f64)); }
    }
    cwj /= n as f64;
    if a != 1.0 && f1 > 0 {
