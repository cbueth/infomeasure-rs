...
fn bayes_entropy_laplace_basic() {
    ...
    let probs = vec![ (2.0+alpha)/denom, (1.0+alpha)/denom, (2.0+alpha)/denom, (1.0+alpha)/denom, (1.0+alpha)/denom ];
    let mut h_exp = 0.0;
    for p in probs { h_exp -= p * p.ln(); }
...
fn bayes_cross_entropy_intersection() {
    ...
    let p1 = 0.5_f64; let p2 = 0.5_f64;
    let q1: f64 = 2.0/7.0; let q2: f64 = 3.0/7.0;
