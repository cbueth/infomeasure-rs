...
#[case(vec![1, 1, 2, 3, 3, 4, 5], "mixed")]
#[case(vec![1, 1, 7, 2, 3, 6, 6, 3], "mixed 2")]
#[case(vec![2, 3, 6, 6, 3, 6, 5, 7], "mixed 3")]
#[case(vec![1, 1, 1, 2, 2, 3], "skewed")]
fn miller_madow_entropy_python_parity(#[case] data: Vec<i32>, #[case] _description: &str) {
    let arr = Array1::from(data.clone());
    let rust_est = MillerMadowEntropy::new(arr);
...
fn test_miller_madow_mi_parity(#[case] x: Vec<i32>, #[case] y: Vec<i32>, #[case] __expected: f64) {
    let x_arr = Array1::from(x.clone());
    let y_arr = Array1::from(y.clone());
    let mi_est = MutualInformation::new_discrete_miller_madow(&[x_arr, y_arr]);
    let res = mi_est.global_value();

    let mi_py = python::calculate_mi(&[x, y], "miller_madow", &[]).unwrap();
...
fn test_miller_madow_te_parity(
    #[case] source: Vec<i32>,
    #[case] dest: Vec<i32>,
    #[case] _expected: f64,
) {
    ...
    let te_est = TransferEntropy::new_discrete_miller_madow(&s_arr, &d_arr, 1, 1, 1);
    ...
    let te_py = python::calculate_te(&source, &dest, "miller_madow", &kwargs).unwrap();
...
fn test_miller_madow_cmi_parity(
    #[case] x: Vec<i32>,
    #[case] y: Vec<i32>,
    #[case] z: Vec<i32>,
    #[case] _expected: f64,
    ...
    let cmi_est = MutualInformation::new_cmi_discrete_miller_madow(&[x_arr, y_arr], &z_arr);
    let res = cmi_est.global_value();

    let cmi_py = python::calculate_cmi(&[x, y], &z, "miller_madow", &[]).unwrap();
...
#[case(vec![1, 1, 1, 1, 1], vec![1, 1, 1, 1, 1], vec![1, 1, 1, 1, 1], 0.0)]
#[case(vec![1, 0, 1, 0], vec![0, 1, 0, 1], vec![1, 1, 0, 0], 0.0)]
fn test_miller_madow_cte_parity(
    #[case] source: Vec<i32>,
    #[case] dest: Vec<i32>,
    #[case] cond: Vec<i32>,
    #[case] _expected: f64,
    ...
    let cte_est =
        TransferEntropy::new_cte_discrete_miller_madow(&s_arr, &d_arr, &c_arr, 1, 1, 1, 1);
    ...
    let cte_py = python::calculate_cte(&source, &dest, &cond, "miller_madow", &kwargs).unwrap();
