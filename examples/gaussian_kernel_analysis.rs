...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &bandwidth in &bandwidths {
        ...
        let python_entropy = python::calculate_entropy_float_nd(&data.as_slice().unwrap(), dims, "kernel", &kernel_kwargs)?;
    ...
    // Create plot
    let root = BitMapBackend::new("gaussian_kernel_comparison.png", (800, 600))
        .into_drawing_area();
    root.fill(&WHITE)?;

    let max_entropy = results.iter()
