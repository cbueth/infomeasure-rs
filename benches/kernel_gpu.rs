        ...
        for dim in dims {
            for size in sizes {
                ...
                let id = BenchmarkId::new(
                    format!("bw{}_dim{}", bw.to_string().replace('.', "_"), dim),
                    size,
                );
                group.bench_with_input(id, &(bw, dim, size), |b, _| {
                    if dim == 1 {
                        let arr = Array1::from(data.clone());
                        b.iter(|| {
                            let entropy = Entropy::new_kernel_with_type(
                                arr.clone(),
                                "gaussian".to_string(),
                                bw,
                            );
                        ...
                        let arr = Array2::from_shape_vec((size, dim), data.clone()).unwrap();
                        b.iter(|| {
                            let entropy = Entropy::nd_kernel_with_type::<{ dim }>(
                                arr.clone(),
                                "gaussian".to_string(),
                                bw,
                            );
