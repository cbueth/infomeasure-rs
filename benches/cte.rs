...
fn bench_discrete_cte(c: &mut Criterion) {
    ...
    let seed = 42u64;
    // Small base = dense joint; base^4 > ~50·N exercises the sparse hash joint
    // (its table is base^4, so the extra base stays at 50 to bound memory).
    let states: [(i32, &str); 2] = [(5, ""), (50, "_b50")];
    ...
    for &(num_states, suffix) in &states {
        for &size in &sizes {
            ...
            group.bench_with_input(id, &size, |b, _| {
                b.iter(|| {
                    black_box(
                        TransferEntropy::cte_discrete_mle(
                            source_arr.as_slice().unwrap(),
                            target_arr.as_slice().unwrap(),
                            cond_arr.as_slice().unwrap(),
                            1,
                            1,
                            1,
                            1,
...
/// Same kernel CTE sweep as `bench_kernel_cte`, executed on a fixed multi-thread
/// rayon pool (see [`PARALLEL_BENCH_THREADS`]) and tracked as
/// `cte_kernel_parallel/...`.
#[cfg(feature = "parallel")]
fn bench_kernel_cte_parallel(c: &mut Criterion) {
    let pool = rayon::ThreadPoolBuilder::new()
        .num_threads(PARALLEL_BENCH_THREADS)
        .build()
        .unwrap();
    let mut group = c.benchmark_group("cte_kernel_parallel");
    group.measurement_time(Duration::from_secs(3));
